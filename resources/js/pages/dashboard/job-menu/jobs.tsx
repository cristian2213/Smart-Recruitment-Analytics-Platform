import { DataTable } from '@/components/datatable/data-table'
import { DataTableHeader } from '@/components/datatable/data-table-header'
import { DataTablePagination } from '@/components/datatable/data-table-pagination'
import AppLayout from '@/layouts/app-layout'
import {
  Job,
  type BreadcrumbItem,
  type HeaderActions,
  type Link,
  type TableData,
} from '@/types'
import { Head } from '@inertiajs/react'
import { VisibilityState, getCoreRowModel, useReactTable } from '@tanstack/react-table'
import { useMemo, useState } from 'react'
import { columns } from './columns'
import { createFormInputs, createJobValidation } from './forms'

const breadcrumbs: BreadcrumbItem[] = [
  {
    title: 'Jobs',
    href: '/dashboard/jobs',
  },
]

const tableHeaderActions: HeaderActions<typeof createJobValidation> = {
  actions: {
    create: {
      userFormInputs: createFormInputs,
      schema: createJobValidation,
      defaultValues: {
        title: '',
        location: '',
        skills: '',
        salary: '',
        status: 'draft',
        placement: '',
        recruiter_id: '',
      },
    },
  },
}

interface JobsProps {
  jobs: {
    data: Job[]
    links: Link[]
  }
}

export default function Jobs({ jobs }: JobsProps) {
  const tableData: TableData<Job> = useMemo(
    () => ({
      columns,
      data: jobs.data,
      links: jobs.links,
    }),
    [jobs.data, jobs.links],
  )
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({})

  const table = useReactTable({
    data: tableData.data,
    columns: tableData.columns,
    getCoreRowModel: getCoreRowModel(),
    onColumnVisibilityChange: setColumnVisibility,
    state: {
      columnVisibility,
    },
  })

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="Users" />
      <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
        <div className="relative min-h-screen flex-1 overflow-hidden rounded-xl border border-sidebar-border/70 px-4 py-5 md:min-h-min dark:border-sidebar-border">
          <div className="flex flex-col overflow-hidden rounded-md">
            <DataTableHeader<Job, typeof createJobValidation>
              table={table}
              headerActions={tableHeaderActions}
            />

            <DataTable<Job> table={table} />

            <div className="mt-4">
              <DataTablePagination links={tableData.links} />
            </div>
          </div>
        </div>
      </div>
    </AppLayout>
  )
}
